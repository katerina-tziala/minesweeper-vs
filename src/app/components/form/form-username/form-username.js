...
export class FormUsername extends Form {
	...
	constructor(submitAction, username) {
		super(submitAction);
		this.inputsGroup.inputControllers = new TextInput(FIELD, username, this.toggleSubmission.bind(this));
	}

	renderFormFields(form) {
		const section = this.renderFormSection();
		section.append(ElementGenerator.generateContainer([DOM_ELEMENT_CLASS.usernameIcon]));
		this.inputsGroup.inputControllers.forEach(inputControler => section.append(inputControler.generateInputField()));
		form.append(section);
	}
