...
import {
  DOM_ELEMENT_ID as APP_ELEMENTS_IDS,
  DOM_ELEMENT_CLASS as APP_STYLE_CLASS,
...
export class SettingsController {
  ...
  init() {
    ...
    this.initColorController(
      SettingType.PlayerColorType,
      SettingType.OpponentColorType,
    );
    this.initColorController(
      SettingType.OpponentColorType,
      SettingType.PlayerColorType,
  ...
  initView() {
    this.settingsContainer.then((container) => {
      const settingsBtn = ElementGenerator.generateButton(
        SETTINGS_BTN,
        this.toggleSettingsDisplay.bind(this),
      );
      const settingsPanel = ElementGenerator.generateContainer(
        [DOM_ELEMENT_CLASS.settingsPanel],
        DOM_ELEMENT_ID.settingsPanel,
      );
      this.setPanelStyle(settingsPanel, 0);
      Object.values(SettingType).forEach((settingType) =>
        settingsPanel.append(this.renderSetting(settingType)),
      );
  ...
  initThemeTypeController() {
    ...
    this.inputsGroup.controllers = new Switcher(
      params,
  ...
  initMineTypeController() {
    ...
    this.inputsGroup.controllers = new DropdownSelect(
      ...
      this.onDropdownExpanded.bind(this),
    );
  }

  initColorController(settingName, settingToCheckName) {
    const params = this.getThemeTypeControllerParams(settingName);
    params.options = SettingsOptionsHelper.getAllowedColorOptions(
      this.settings[settingToCheckName],
    );
    this.inputsGroup.controllers = new DropdownSelect(
      params,
      this.onDropdownChange.bind(this),
  ...
  renderSetting(key) {
    ...
    settingsSection.append(
      this.inputsGroup.getController(key).generateInputField(),
    );
  ...
  expandSettings() {
    ...
    this.settingsPanel.then((panel) => {
      ...
      height += roundUpToNextDecade(
        settingsSections[0].getBoundingClientRect().height,
      );
  ...
  getGameSettingsHeight(settingsSections) {
    ...
    for (let index = 1; index < settingsSections.length; index++) {
      ...
      if (this.gameSettingsHidden) {
        ...
        height += roundUpToNextDecade(
          settingsSection.getBoundingClientRect().height,
        );
      }
    }
  ...
  collapseOnOutsideClick(event) {
    ...
    document.removeEventListener(
      "click",
      this.collapseOnOutsideClick.bind(this),
    );
  }
  ...
  onDropdownExpanded(expandedName) {
    ...
    toCollapse.forEach((type) =>
      this.inputsGroup.getController(type).collapseDropdown(),
    );
  }
  ...
  setAppTheme() {
    ...
    ElementHandler.getByID(APP_ELEMENTS_IDS.app).then(
      (appContainer) => (appContainer.className = appStyles),
    );
  }
  ...
  updateColorDropdown(changedField, fieldToUpdate) {
    const newOptions = SettingsOptionsHelper.getAllowedColorOptions(
      this.settings[changedField],
    );
