...
import {
  ...
  DOM_ELEMENT_CLASS,
  DROPDOWN_BNT,
} from "./dropdown-select-button.constants";
...
export class DropdownSelectButton {
  ...
  generateDropdownBtn(params) {
    const button = ElementGenerator.generateButton(
      DROPDOWN_BNT,
      this.onDropdownClick.bind(this),
    );
    ...
    this.dropdownBtn.then((button) => {
      switch (keyCode) {
        ...
        case 38: // Up Arrow
          ...
          break;
        case 13: // Enter
          this.toggleButtonStateAndSubmit(
            button,
            !AriaHandler.getAriaExpanded(button),
          );
          break;
      }
    });
  ...
  enable() {
    this.dropdownBtn.then((button) =>
      ElementHandler.setDisabled(button, false),
    );
  }

  setButtonLabel(displayValue) {
    this.dropdownBtnLabel.then(
      (btnLabel) => (btnLabel.innerHTML = displayValue),
    );
  }
  ...
  toggleButtonExpandState(expandState) {
    this.dropdownBtn.then((button) =>
      this.toggleButtonState(button, expandState),
    );
  }
}
